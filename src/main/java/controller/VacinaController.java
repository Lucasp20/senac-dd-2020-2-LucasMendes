...
import model.exception.VacinaJaCadastradaException;
...
public class VacinaController {
	...
	public String cadastrarVacina(Vacina vacina) {
		...
		if(validarPais(vacina) 
				&& validarDataInicioPesquisa(vacina) 
				&& validarEstagio(vacina)
	...
	private boolean validarPesquisador(Vacina vacina) {
		if(vacina.getPesquisador().length() < 3
				|| vacina.getPesquisador().isEmpty() ) {
